...
string argument_node_render(argument_node* arg, render_info* info) {
	...
	if (info->format.argFormat == ARGUMENT_FORMAT_DEFAULT || info->format.argFormat == ARGUMENT_FORMAT_C) {
...
void argument_node_get_fields(argument_node* arg, list* fields, format_options* format) {
	if (format->argFormat == ARGUMENT_FORMAT_DEFAULT || format->argFormat == ARGUMENT_FORMAT_C) {
