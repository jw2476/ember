    ...
    if (strcmp(input, "fn") == 0) {
        ...
        if (info->currentField->parent->type == NODE_FUNCTION) {
            ...
            string_append(&buffer->data,
                          string_new("Arguments can only be inserted to "
                                     "function or argument nodes"));
        }
        ...
        if (info->currentField->parent->type == NODE_CALL) {
            call_node* parent = (call_node*)info->currentField->parent;
            string_literal_node* node =
                string_literal_node_new((node_generic*)parent);
            list_append(parent->parameters, node);
...
void add_modifier(input_buffer* buffer, render_info* info, const char* input) {
    if (strcmp(input, "extern") == 0) {
        if (info->currentField->parent->type == NODE_FUNCTION) {
            ...
            string_clear(&buffer->data);
            string_append(&buffer->data,
                          string_new("Only functions can be extern"));
        }
    }
}
...
void handle_input(input_buffer* buffer, render_info* info) {
    ...
    if (buffer->mode != INPUT_MODE_OFF) {
        ...
        if (IsKeyPressed(KEY_BACKSPACE) &&
            buffer->data.size > buffer->protected) {
            string_remove(&buffer->data, buffer->data.size - 1);
        }

        if (IsKeyPressed(KEY_ENTER)) {
            string inputStr = string_substr(&buffer->data, buffer->protected,
                                            buffer->data.size);
...
int main() {
    ...
    Font regular = LoadFontEx("assets/JetBrainsMono/JetBrainsMono-Regular.ttf",
                              fontSize, NULL, 0);

    render_info info = render_info_new();

    input_buffer inputBuffer =
        (input_buffer){string_new(""), 0, INPUT_MODE_OFF};
    ...
    while (!WindowShouldClose()) {
        ...
        if (inputBuffer.mode == INPUT_MODE_OFF && info.currentField != NULL) {
            ...
            if (IsKeyPressed(KEY_BACKSPACE) &&
                info.currentField->data.size != 0) {
                string_remove(&info.currentField->data,
                              info.currentField->data.size - 1);
            }
        ...
        BeginDrawing();
        {
            ...
            DrawTextEx(regular, outputText, (Vector2){190, 200}, fontSize, 0,
                       RAYWHITE);

            DrawRectangle(0, GetScreenHeight() - (fontSize + 32),
                          GetScreenWidth(), fontSize + 32, ZINC800);
            DrawTextEx(regular, inputBufferText,
                       (Vector2){16, GetScreenHeight() - (fontSize + 16)},
                       fontSize, 0, RAYWHITE);
        }
        EndDrawing();
